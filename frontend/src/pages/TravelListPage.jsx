...
export default function TravelListPage() {
  ...
  const load = useCallback(() => {
    api(`/travel-lists/${id}/`)
      .then(setList)
      .catch((err) => setError(err.status === 404 ? t("Маршрут не найден или он закрыт автором.") : err.message));
  }, [id]);
  useEffect(load, [load]);
  ...
  async function share() {
    try {
      ...
      toast(t("Ссылка на маршрут скопирована"));
    } catch {
  ...
  async function copy() {
    if (!requireLogin(t("Войдите, чтобы скопировать маршрут"))) return;
    try {
      const copyList = await api(`/travel-lists/${list.id}/copy/`, { method: "POST" });
      toast(t("Маршрут «{0}» добавлен в ваши списки", copyList.title));
    } catch (err) {
  ...
  return (
    <div className="flex flex-col w-full bg-surface">
            ...
            <div className="flex gap-2">
              ...
              {!isOwner && (
                <button className="bg-primary hover:bg-tertiary-container text-on-primary font-semibold text-body-sm px-4 py-2.5 rounded-lg flex items-center gap-1.5" onClick={copy} type="button">
                  <Icon name="content_copy" className="text-[18px]" /> {t("Скопировать себе")}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full py-8 flex flex-col gap-8">
        ...
        <section className={`${card} flex flex-col gap-4`}>
          <h2 className="text-headline-md font-headline-md text-on-surface">{t("Остановки")}</h2>
          {items.length === 0 && <p className="text-body-md text-on-surface-variant">{t("В маршруте пока нет мест.")}</p>}
                  ...
                  <div className="flex items-center gap-4 bg-surface-container-lowest/60 rounded-xl p-3">
                    ...
                    {isOwner ? (
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">{item.is_visited ? t("Посещено") : t("Отметить")}</span>
                      </button>
                    ) : (
