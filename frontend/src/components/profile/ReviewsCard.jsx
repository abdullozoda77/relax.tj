...
export default function ReviewsCard({ onChanged }) {
  ...
  async function remove(review) {
    if (!confirm(`Удалить отзыв о месте «${review.place_name}»?`)) return;
    try {
    ...
    <div className="bg-surface-container rounded-xl p-6 lg:p-7 shadow-xl flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-title-md text-title-md text-on-surface">Мои отзывы</h3>
        </div>
        <span className="font-label-sm text-label-sm text-primary">{reviews ? reviews.length : "…"} ВСЕГО</span>
      </div>
      <div className="flex flex-col gap-3">
        {reviews?.length === 0 && <p className="text-body-sm text-on-surface-variant">Вы ещё не писали отзывов.</p>}
        {reviews?.map((r) => (
          <div key={r.id} className="bg-surface-container-low hover:bg-surface-container-high transition-colors p-4 rounded-xl flex items-center justify-between gap-2">
            ...
            <div className="flex shrink-0">
              <button className="text-on-surface-variant hover:text-primary transition-colors p-2" onClick={() => openPlace(r.place)} title="Открыть место" type="button">
                <Icon name="visibility" className="text-[20px]" />
              </button>
              <button className="text-on-surface-variant hover:text-rose-400 transition-colors p-2" onClick={() => remove(r)} title="Удалить отзыв" type="button">
      ...
      <div className="bg-surface-container-lowest p-3 rounded-lg flex items-center gap-3">
        ...
        <p className="font-body-sm text-body-sm text-on-surface-variant">Ваши отзывы помогают другим путешественникам выбрать место для отдыха.</p>
      </div>
    </div>
  );
}
