  ...
  return (
    <div className="bg-surface-container rounded-xl p-6 lg:p-8 shadow-xl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <span className="font-label-sm text-label-sm text-secondary uppercase tracking-wider">ИЗБРАННОЕ</span>
          <h2 className="font-headline-md text-headline-md text-on-surface mt-1">Сохранённые места</h2>
        </div>
      ...
      {favorites?.length === 0 && (
        <p className="text-body-md text-on-surface-variant">Пока пусто. Нажмите ♥ на карточке места, чтобы сохранить его сюда.</p>
      )}
          ...
          >
            <div className="relative h-36 overflow-hidden">
              ...
              <span className="absolute top-2 right-2 bg-surface-container-lowest/80 backdrop-blur-md text-primary font-label-sm text-label-sm px-2 py-0.5 rounded">
                {p.category || "Место"} · {SEASONS[p.best_season]}
              </span>
            </div>
