...
`python manage.py seed` заполняет базу: 5 регионов, 9 категорий, 7 активностей и 64 места по всему
Таджикистану — от Душанбе и Фанских гор до Памира и Хатлона (Искандеркуль, Семь озёр, Сарез, Каракуль,
Хорог, Пенджикент, Нурек и др.). Повторный запуск не создаёт дубликаты. Координаты мест взяты из
Wikidata / Wikipedia, высоты — из Wikidata или Open-Meteo; цены входа примерные.
...
- `/` — главная: места с фильтрами и поиском, места рядом (геолокация), 3D-карта всех мест
  (загружается по кнопке, клик по метке — перелёт к месту), категории,
...
- `/lists/<id>` — страница маршрута: остановки по порядку с расстояниями, общая длина,
  высшая точка, 3D-карта с номерами мест и линией маршрута, «Поделиться» и «Скопировать себе».
...
Языки интерфейса: русский, таджикский и английский — переключатель RU / TJ / EN в шапке
(выбор сохраняется, страница перезагружается). Тексты переводятся функцией `t()` из `src/i18n.js`,
переводы лежат в `src/translations.js` (ключ — русский текст). Названия регионов, категорий и
активностей тоже переводятся, а описания мест из базы данных остаются на русском.
Светлая и тёмная тема — кнопка ☀/🌙 в шапке.
...
| `i18n.js`, `translations.js` | Языки RU / TJ / EN |
